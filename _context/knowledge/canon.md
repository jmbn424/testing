...
- GitHub Pages is ruled out as the static host (its commercial-use terms bar a business web presence). The canonical host is Cloudflare Pages (confirmed 2026-06-10; Free plan sufficient — see the Cloudflare entry, rev 16); GitHub holds the private source repo and CI only. Decision rationale (dated 2026-06-12): repo changes drive the build, Cloudflare Pages performs the production deploy — this is what keeps the commercial site off GitHub Pages’ prohibited-use terms while retaining Git-driven CI. See company-facts Web for the legally-relevant properties.
...
- **5.35** Cloudflare Pages preview-deployment exposure (rev 16; Cloudflare docs captured 2026-06-12) — Pages allows an UNLIMITED number of active preview deployments, each at its own public URL. For a regulated TPMO site that means an unreviewed branch (draft disclaimers, pre-counsel legal pages, an ON-state SMS form before the program is registered) can sit live at a shareable URL. Mitigation: treat preview URLs as confidential, do not share casually, and ensure previews are noindex / robots-excluded so draft regulated copy is not indexed. Pairs with build-output-strip (annotations must already be stripped before any preview is shareable) and the launch-gate switches. Operational, not a copy change.
